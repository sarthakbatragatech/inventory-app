...
function Photo({ row }: { row: Material }) {
  return row.photoUrl ? <a href={row.photoUrl} target="_blank" rel="noopener noreferrer" className={s.photo} aria-label={`View photo of ${row.componentName}`}>
    <Image src={row.photoUrl} alt={row.componentName} width={120} height={100} sizes="60px" />
...
export function ProductionMaterials({ dashboard, colors, initialStatus = 'all' }: {
  ...
  const rows = useMemo(() => allRows.filter(row => {
    ...
    const matchesStatus = status === 'all' || (status === 'purchase' && (row.shortageForOpenOrdersQty ?? 0) > 0) || (status === 'negative' && row.availableQty < 0) || (status === 'zero' && row.buildableQty === 0);
    ...
    return Number(b.availableQty < 0) - Number(a.availableQty < 0) || Number(b.buildableQty === 0) - Number(a.buildableQty === 0) || (b.shortageForOpenOrdersQty ?? 0) - (a.shortageForOpenOrdersQty ?? 0) || a.buildableQty - b.buildableQty;
  }), [allRows, scope, search, sort, status]);
  const purchaseCount = allRows.filter(row => (row.shortageForOpenOrdersQty ?? 0) > 0).length;

  function exportRows() {
    csvDownload(`FR-CRUZER-materials-${new Date().toISOString().slice(0, 10)}.csv`, [
      ['Component', 'SKU', 'Colours', 'Stage', 'Per bike', 'Unit', 'Inward', 'Consumed', 'Available', 'Part capacity (bikes)', 'Net requirement', 'To order', 'Last inward date', 'Last inward quantity', 'Last inward unit'],
      ...rows.map(row => [row.componentName, row.componentSku, row.colors?.join('; ') || 'Shared', row.consumptionStage, row.qtyPerFg, row.unit, row.inwardQty, row.consumedQty, row.availableQty, row.buildableQty, row.requiredForOpenOrdersQty, row.shortageForOpenOrdersQty, row.lastInwardDate, row.lastInwardQty, row.lastInwardUnit]),
    ]);
  }

  return <section className={s.panel} aria-labelledby="materials-heading">
    <div className={s.panelHeading}><div><h2 id="materials-heading">Materials &amp; readiness</h2><p>{allRows.length} components · {purchaseCount} shared parts to order</p></div><button className={s.button} onClick={exportRows} disabled={!rows.length}>Export CSV <span aria-hidden="true">↓</span></button></div>
    <div className={s.filters}>
      ...
      <label><span className={s.srOnly}>Stock status</span><select value={status} onChange={e => setStatus(e.target.value)}><option value="all">All stock</option><option value="purchase">To order</option><option value="negative">Negative stock</option><option value="zero">Zero capacity</option></select></label>
    ...
    {rows.length ? <>
      <div className={s.desktopMaterials}>
        <table className={s.materialTable}><caption className={s.srOnly}>Cruzer components, stock, net requirements and last inward</caption><thead><tr><th scope="col"><span className={s.srOnly}>Photo</span></th><th scope="col">Component</th><th scope="col">Per bike</th><th scope="col" className={s.numeric}>Stock</th><th scope="col" className={s.numeric}>Part capacity</th><th scope="col" className={s.numeric}>Net need</th><th scope="col" className={s.numeric}>To order</th><th scope="col" className={s.numeric}>Last inward</th></tr></thead><tbody>
          {rows.map(row => <tr key={row.componentItemId} className={row.availableQty < 0 ? s.negativeRow : undefined}>
            ...
            <td className={`${s.numeric} ${row.availableQty < 0 ? s.danger : ''}`} title={`${quantity(row.inwardQty)} inward − ${quantity(row.consumedQty)} consumed`}>{quantity(row.availableQty)}</td>
            <td className={s.numeric}>{quantity(row.buildableQty)}</td><td className={s.numeric}>{quantity(row.requiredForOpenOrdersQty)}</td>
            <td className={s.numeric}>{row.shortageForOpenOrdersQty == null ? <span className={s.muted} title="Assign an order colour mix to calculate">Unassigned</span> : <span className={row.shortageForOpenOrdersQty > 0 ? s.shortage : s.muted}>{quantity(row.shortageForOpenOrdersQty)}</span>}</td>
      ...
      <div className={s.mobileMaterials}>{rows.map(row => <article key={row.componentItemId} className={`${s.materialCard} ${row.availableQty < 0 ? s.negativeRow : ''}`}>
        ...
        <dl className={s.materialNumbers}><div><dt>Stock</dt><dd className={row.availableQty < 0 ? s.danger : ''}>{quantity(row.availableQty, row.unit)}</dd></div><div><dt>Per bike · {row.consumptionStage === 'packed' ? 'packing' : 'assembly'}</dt><dd>{quantity(row.qtyPerFg, row.unit)}</dd></div><div><dt>Part capacity</dt><dd>{quantity(row.buildableQty)} bikes</dd></div><div><dt>Net need</dt><dd>{quantity(row.requiredForOpenOrdersQty, row.unit)}</dd></div><div><dt>To order</dt><dd className={(row.shortageForOpenOrdersQty ?? 0) > 0 ? s.danger : ''}>{row.shortageForOpenOrdersQty == null ? 'Unassigned' : quantity(row.shortageForOpenOrdersQty, row.unit)}</dd></div></dl>
