...
- Cruzer production planning uses a read-time kg-to-pieces estimate when a lot has no piece count: its own recorded average first, otherwise the most recent strictly earlier recorded average for the same component and colour. Measured kg/pieces ratios take priority over rounded average cells; estimates round down per lot and never seed later estimates. The materials table, CSV, capacity and alert preview label these values as provisional and expose their reference lot. This does not rewrite imported records or the general accounting stock views; importing corrected piece counts replaces the estimates. Physical reconciliation removes earlier estimates from the current-stock basis.
